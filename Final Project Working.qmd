...
ggplot(data = x500_long, aes(x = Interval, y = Time)) +
  geom_point(aes(group = Trial, color = factor(Trial))) +
  geom_line(data = x500_line, aes(x = Interval, y = Mean, 
                                   group = Trial, color = factor(Trial)),
            linewidth = 1.5) +
  scale_color_manual(values = c("purple", "blue", "green3"), name = "Trial") +
...
ggplot(data = pred_10k_2k, aes(x = second10k, y = k2)) +
  facet_wrap(~ class_year) +
  geom_point(aes(color = factor(v1)), alpha = 0.6) +
  geom_line(
    data = pred_time,
    aes(x = second10k, y = predicted, group = interaction(class_year, v1), color = factor(v1)),
    linewidth = 1) +
